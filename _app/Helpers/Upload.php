...
/**
 * Upload.class[ HELPER ]
 * Classe responsável por executar uploads de imagens, arquivos e mídias no sistema.
...
class Upload {
    ...
    function __construct($BaseDir = null){
        self::$BaseDir = ((string) $BaseDir ? $BaseDir : '../../uploads/' ); // com ../uploads/ ele cria a pasta dentro do diretório raiz do site,
        //ou seja, dentro da pasta modulos. para criar dentro da pasta que voce esta utilizando, que é a classe auxiliares, tem que informar o
        // nome da pasta no construtor.
     ...
     * Caso nÃ£o informe o tamanho será 2mb!
    ...
    public function File(array $File, $Name = null, $Folder = null, $MaxFileSize = null){
        ...
        if($this->File['size'] > ($MaxFileSize * (1024*1024))){
            $this->Result = false;
            $this->Error = "Arquivo muito grande, tamanho máximo permitido {$MaxFileSize}mb";
        }
        elseif(!in_array($this->File['type'], $FileAccept)){
            $this->Result = false;
            $this->Error = "Tipo de arquivo não suportado. Envie .PDF!";
        }
        else{
            $this->CheckFolder($this->Folder);
            $this->SetFileName();
    ...
    /**
     * <b>Verificar Upload:</b> Executando um getResult é possíel verificar se o Upload foi executado ou não. Retorna

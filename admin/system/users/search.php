    ...
    <div class="container padding20 bg-write breadcrumbs">
        ...
        <div class="form-search fl-right">
            <form class="container no_post_ajax" method="post" action="">
                <input style="width:350px; margin-top: 10px; margin-right: 8px;" type="text" name="s" value="<?= (isset($getSearch) && !empty($getSearch) ? $getSearch : ''); ?>" placeholder="Pesquisar Usuário" required/>
...
<div class="content form_create usuarios">
    ...
    <section class="container">
        ...
        <?php 
            ...
            if(!empty($getSearch)){
                $getSearch = strip_tags(trim($getSearch));
                if(strstr($getSearch, " ")){
                    $getSearch = explode(" ", $getSearch);
                    $readUsers->ExeRead("ws_users", "WHERE user_name = :user_name AND user_lastname = :user_lastname", "user_name={$getSearch[0]}&user_lastname={$getSearch[1]}");
                    $getSearch = implode(" ", $getSearch);
                }
                else{
                    $readUsers->ExeRead("ws_users", "WHERE user_name LIKE '%' :s '%' OR user_lastname LIKE '%' :s '%'", "s={$getSearch}");
                }
            }    
            else{
                header("location:painel.php?cc=users/users");
            ...
            if($readUsers->getResult()){
                foreach($readUsers->getResult() as $users){
                    ...
                    $u++;
            ?>  
                <article class="users bottom30 padding20 bg-write box box-medium <?php if($u % 3 === 0): echo "last"; endif; ?>">
                    <ul>    
                        ...
                        <li class="bottom5 ds-block"><span class="fontsizeb"><?php if($user_level == 1){ echo 'CLIENTE NOVO<p>(Usuário)</p>'; } elseif($user_level == 2){ echo 'Editor'; } else{ echo 'SUPER ADMIN<p>(Admin)</p>'; } ?></span></li>
            ...
            <?php 
                ...
                echo WSErro("Desculpe, não existem usuários cadastradas no seu sistema com termo <b>{$getSearch}</b>!", 'bg-green');
            }
            ?>
